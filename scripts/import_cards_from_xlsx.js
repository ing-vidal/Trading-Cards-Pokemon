...
(async () => {
  ...
  try {
    ...
    try {
      XLSX = require('../apps/admin/node_modules/xlsx');
    } catch (fallbackErr) {
  ...
  const imagesFolder = argv[1] ? path.resolve(argv[1]) : path.resolve('assets/card-images');
  ...
  try {
    const existingRes = await fetch(`${API}/api/cards?limit=1000`);
    if (existingRes.ok) {
      const existingJson = await existingRes.json();
      for (const card of existingJson.data || []) {
        if (card.number && card.id) existingCardsByNumber.set(String(card.number), card.id);
      }
    }
  } catch (err) {
    console.warn('Could not load existing cards for number-based updates:', err.message || err);
  }
  ...
  for (const r of rows) {
    ...
    // Use column H when present; otherwise try the card number as the filename.
    if (imagesFolder && fs.existsSync(imagesFolder)) {
      const requestedName = String(r.imageFilename || r.number || '').trim();
      const requestedBase = path.basename(requestedName).replace(/\.[^.]+$/, '').toLowerCase();
      const imageName = fs.readdirSync(imagesFolder).find((name) => {
        const fileBase = name.replace(/\.[^.]+$/, '').toLowerCase();
        return name.toLowerCase() === requestedName.toLowerCase() || fileBase === requestedBase;
      });
      const imgPath = imageName ? path.join(imagesFolder, imageName) : null;
      ...
      if (imgPath) {
        try {
          ...
          if (upRes.ok) {
            ...
            console.warn('Image upload failed for', imgPath, 'status', upRes.status, await upRes.text());
          }
        } catch (err) {
        ...
        missingImages += 1;
        console.warn('Image file not found for', r.number, 'in', imagesFolder);
      }
    }

    try {
      let res;
      const existingId = r.id && r.id.toString().trim()
        ? r.id.toString().trim()
        : existingCardsByNumber.get(String(r.number));
      if (existingId) {
        res = await fetch(`${API}/api/cards/${existingId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        const body = res.ok ? await res.json() : await res.text();
        console.log('PUT', r.number, '=>', res.status, typeof body === 'object' ? JSON.stringify(body).slice(0,200) : body);
      } else {
  ...
  console.log(`Import finished. Images uploaded: ${uploadedImages}; images not found: ${missingImages}.`);
})();
