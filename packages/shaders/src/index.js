...
exports.ALL_PRESETS = exports.TRAINER_GALLERY_PRESET = exports.SPECIAL_ART_PRESET = exports.PROMO_GLOW_PRESET = exports.GLASS_SHATTER_PRESET = exports.DOUBLE_IMMERSIVE_RARE_PRESET = exports.IMMERSIVE_RARE_PRESET = exports.GOLD_RELIC_PRESET = exports.RAINBOW_HYPER_PRESET = exports.TWO_STAR_FOIL_PRESET = exports.ONE_STAR_FOIL_PRESET = exports.DOUBLE_RARE_FOIL_PRESET = exports.RARE_FOIL_PRESET = exports.BASIC_FOIL_PRESET = void 0;
exports.getPresetById = getPresetById;
exports.createShaderUniforms = createShaderUniforms;
// ─── BASIC FOIL ─────────────────────────────────────────────────────────────
exports.BASIC_FOIL_PRESET = {
    id: 'basic-foil',
...
// ─── DOUBLE RARE FOIL ───────────────────────────────────────────────────────
exports.DOUBLE_RARE_FOIL_PRESET = {
    id: 'double-rare-foil',
...
// ─── TWO STAR FOIL ───────────────────────────────────────────────────────────
exports.TWO_STAR_FOIL_PRESET = {
    id: 'two-star-foil',
    name: 'Two Star Aurora Prism Foil',
    description: 'Marco arcoíris intenso con aurora pastel, haces diagonales blancos y destellos de dos estrellas.',
    vertexShader: `
    varying vec2 vUv;
    varying vec3 vNormal;
    varying vec3 vViewPosition;
  ...
  `,
    ...
    void main() {
      ...
      vec2 centered = vUv - 0.5;
      float angle = dot(uMousePos, vec2(1.1, 0.8));
      float motion = uTime * 0.12 + angle * 0.35;

      float diagonal = dot(centered, vec2(0.76, 0.65));
      float rayA = exp(-pow((diagonal - motion * 0.16) * 9.0, 2.0));
      float rayB = exp(-pow((dot(centered, vec2(-0.62, 0.92)) + motion * 0.12) * 10.0, 2.0));
      float rayC = exp(-pow((dot(centered, vec2(0.35, -0.94)) - motion * 0.1) * 13.0, 2.0));
      float auroraWave = 0.5 + 0.5 * sin(vUv.x * 10.0 + vUv.y * 7.0 + motion);
      vec3 cosmic = cosmicColor(diagonal * 0.42 + auroraWave * 0.24 + fresnel * 0.35 + angle * 0.08);
      ...
      float stars = starBurst(vUv + uMousePos * 0.025 + vec2(uTime * 0.003, -uTime * 0.002), 0.7);
      stars += starBurst(vUv * 1.7 - uMousePos * 0.018, 1.4) * 0.7;

      float borderEdge = min(min(vUv.x, 1.0 - vUv.x), min(vUv.y, 1.0 - vUv.y));
      float border = smoothstep(0.085, 0.018, borderEdge);
      float edgeFacing = 1.0 - abs(vNormal.z);
      float edgeReflection = pow(edgeFacing, 1.15) * (0.4 + 0.6 * facing);
      vec3 edgeColor = cosmicColor((vUv.x + vUv.y) * 0.7 + angle * 0.2);
      ...
      finalColor += cosmic * (rayA * 0.52 + rayB * 0.42 + rayC * 0.3) * uIntensity;
      finalColor += vec3(1.0, 0.98, 0.94) * (rayA * rayA * 0.55 + rayB * rayB * 0.45) * uIntensity;
      finalColor += cosmic * border * 0.62 * uIntensity;
      finalColor += vec3(1.0, 0.96, 1.0) * (particleField * 0.3 + stars * 1.2 + mouseHighlight * 0.2) * uIntensity;
...
exports.GOLD_RELIC_PRESET = {
    id: 'gold-relic',
    name: 'Gold Ultra Rare Metallic',
    description: 'Carta dorada Ultra Rare con iridiscencia rainbow, chispas animadas en forma de estrella, barrido de luz metálica y textura de glitter.',
    vertexShader: `
    varying vec2 vUv;
    varying vec3 vNormal;
    varying vec3 vViewPosition;
...
// ─── IMMERSIVE RARE ──────────────────────────────────────────────────────────
exports.IMMERSIVE_RARE_PRESET = {
    id: 'immersive-rare',
    name: 'Immersive Rare Living Illustration',
    description: 'Ilustración inmersiva con parallax cromático, profundidad por capas, brillo prismático y destellos reactivos al movimiento.',
    vertexShader: `
    varying vec2 vUv;
    varying vec3 vNormal;
    varying vec3 vViewPosition;
...
exports.DOUBLE_IMMERSIVE_RARE_PRESET = {
    ...exports.IMMERSIVE_RARE_PRESET,
    id: 'double-immersive-rare',
    name: 'Double Immersive Starburst Foil',
    description: 'Holofoil inmersivo de doble capa con marco arcoíris intenso, barrido diagonal y destellos estelares concentrados.',
    uniformsDefaults: {
        ...exports.IMMERSIVE_RARE_PRESET.uniformsDefaults,
        uDoubleIntensity: 1,
    },
};
// ─── DIAMOND / GLASS SHATTER ─────────────────────────────────────────────────
exports.GLASS_SHATTER_PRESET = {
    id: 'glass-shatter',
    name: 'Diamond Shattered Crystal',
    description: 'Refracción de cristal de diamante con facetas prismáticas y borde de luz que recorre la carta.',
    vertexShader: `
    varying vec2 vUv;
    varying vec3 vNormal;
    varying vec3 vViewPosition;
...
exports.SPECIAL_ART_PRESET = {
    id: 'special-art',
    name: 'Special Illustration Rare Galaxy',
    description: 'Efecto cosmos profundo con nebulosa animada, estrellas flotantes y gradiente espacial.',
    vertexShader: `
    varying vec2 vUv;
    varying vec3 vNormal;
    varying vec3 vViewPosition;
...
exports.TRAINER_GALLERY_PRESET = {
    id: 'trainer-gallery',
...
function getPresetById(id) {
    return exports.ALL_PRESETS.find((p) => p.id === id) || exports.BASIC_FOIL_PRESET;
}
function createShaderUniforms(presetId, customUniforms) {
