...
exports.GoldCard3DCanvas = GoldCard3DCanvas;
/**
...
const RoundedBoxGeometry_js_1 = require("three/examples/jsm/geometries/RoundedBoxGeometry.js");
...
function GoldCard3DCanvas({ imageUrl, intensity = 0.9, width = '100%', height = '520px', showEditor = false, variant = 'gold', }) {
    const initialParams = variant === 'crown' ? CROWN_PARAMS : DEFAULT_PARAMS;
    const containerRef = (0, react_1.useRef)(null);
    const paramsRef = (0, react_1.useRef)({ ...initialParams, goldIntensity: intensity * initialParams.goldIntensity });
    const materialRef = (0, react_1.useRef)(null);
    const physMatRef = (0, react_1.useRef)(null);
    const composerRef = (0, react_1.useRef)(null);
    // UI state for editor
    const [editorOpen, setEditorOpen] = (0, react_1.useState)(showEditor);
    const [params, setParams] = (0, react_1.useState)({ ...initialParams });
    // Update shader uniforms when params change
    const updateUniforms = (0, react_1.useCallback)((p) => {
    ...
    const setParam = (0, react_1.useCallback)((key, val) => {
        setParams(prev => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        let cardGeo;
        try {
            cardGeo = new RoundedBoxGeometry_js_1.RoundedBoxGeometry(CARD_W, CARD_H, CARD_D, 4, 0.06);
        }
        catch {
        ...
        const bloom = new UnrealBloomPass_js_1.UnrealBloomPass(new THREE.Vector2(w, h), p.glowIntensity, // strength
        0.35, // radius
        p.bloomThreshold);
    ...
    return (<div style={{ position: 'relative', width, height }}>
      ...
      {editorOpen && (<div style={{
          ...
          {sliders.map(({ key, label, min, max, step }) => (<div key={key} style={{ marginBottom: '7px' }}>
              ...
              <input type="range" min={min} max={max} step={step} value={params[key]} onChange={e => setParam(key, parseFloat(e.target.value))} style={{ width: '100%', accentColor: '#ffd060', height: '3px' }}/>
            </div>))}
          <button onClick={() => { setParams({ ...DEFAULT_PARAMS }); paramsRef.current = { ...DEFAULT_PARAMS }; updateUniforms(DEFAULT_PARAMS); }} style={{ marginTop: '6px', width: '100%', background: '#ffd06022', border: '1px solid #ffd06055', color: '#ffd060', borderRadius: '5px', padding: '4px', fontSize: '0.65rem', cursor: 'pointer', fontWeight: 700 }}>
            Reset to Default
          </button>
        </div>)}
    </div>);
