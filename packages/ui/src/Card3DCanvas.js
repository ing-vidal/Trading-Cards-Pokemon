...
exports.Card3DCanvas = Card3DCanvas;
...
const GoldCard3DCanvas_1 = require("./GoldCard3DCanvas");
...
function Card3DCanvas({ presetId = 'basic-foil', intensity = 0.75, imageUrl, width = '100%', height = '520px', }) {
    // ── Premium metallic renderers ─────────────────────────────────────────────
    if (presetId === 'gold-relic' || presetId === 'crown-rare') {
        return (<GoldCard3DCanvas_1.GoldCard3DCanvas imageUrl={imageUrl} intensity={intensity} width={width} height={height} showEditor={false} variant={presetId === 'crown-rare' ? 'crown' : 'gold'}/>);
    }
    const containerRef = (0, react_1.useRef)(null);
    const [isHovered, setIsHovered] = (0, react_1.useState)(false);
    const materialRef = (0, react_1.useRef)(null);
    const cardMeshRef = (0, react_1.useRef)(null);
    ...
    (0, react_1.useEffect)(() => {
        ...
        const geometry = new RoundedBoxGeometry_js_1.RoundedBoxGeometry(2.5, 3.5, 0.12, 0.08, 6);
        const preset = (0, shaders_1.getPresetById)(presetId);
        const initialTexture = new THREE.DataTexture(new Uint8Array([255, 255, 255, 255, 255, 255]), 1, 1, THREE.RGBFormat);
    ...
    return (<div ref={containerRef} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{
            width,
            height,
            position: 'relative',
            borderRadius: '16px',
