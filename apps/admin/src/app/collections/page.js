...
exports.default = CollectionsAdminPage;
...
const normalizeCollectionImages = (value) => {
    if (Array.isArray(value)) {
        return value.filter((item) => typeof item === 'string' && Boolean(item)).slice(0, 3);
    }
    if (typeof value === 'string') {
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                return parsed.filter((item) => typeof item === 'string' && Boolean(item)).slice(0, 3);
            }
        }
        catch {
...
const mergeCollections = (apiCollections, localCollections) => {
...
const persistCollections = (updated) => {
    try {
...
const persistDeletedCollections = (deletedIds) => {
    try {
...
function CollectionsAdminPage() {
    const [collections, setCollections] = (0, react_1.useState)(INITIAL_COLLECTIONS);
    const [searchTerm, setSearchTerm] = (0, react_1.useState)('');
    // Modals state
    const [showCreateModal, setShowCreateModal] = (0, react_1.useState)(false);
    const [editingCol, setEditingCol] = (0, react_1.useState)(null);
    const [deletingCol, setDeletingCol] = (0, react_1.useState)(null);
    // Form State
    ...
    const [previewLogo, setPreviewLogo] = (0, react_1.useState)(null);
    ...
    const handleEditSubmit = async (e) => {
        e.preventDefault();
        if (!editingCol || !formData.name || !formData.code)
            return;
        setIsSubmitting(true);
        const payload = {
            name: formData.name,
        ...
        if (!updatedApi) {
            setCollections((prev) => {
                const updatedList = prev.map((c) => c.id === editingCol.id
                    ? {
                        ...c,
                        name: formData.name,
                        code: formData.code.toUpperCase(),
    ...
    const filteredCollections = collections.filter((col) => col.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', paddingBottom: '3rem' }}>
      ...
      <div style={{
        ...
        <input type="text" placeholder="🔍 Buscar colección por nombre, código o slug..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{
            flex: 1,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
        {filteredCollections.length === 0 ? (<div style={{ gridColumn: '1 / -1', padding: '3rem', textAlign: 'center', backgroundColor: '#18181b', borderRadius: '12px', color: '#71717a' }}>
            No se encontraron colecciones.
          </div>) : (filteredCollections.map((col) => (<div key={col.id} style={{
                backgroundColor: '#18181b',
                border: '1px solid #27272a',
                borderRadius: '14px',
                padding: '1.5rem',
              ...
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                {col.images && col.images.length > 0 ? (<div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap', width: '60px' }}>
                    {col.images.slice(0, 3).map((image, index) => (<img key={`${col.id}-${index}`} src={image} alt={`${col.name} ${index + 1}`} style={{
                        width: '60px',
                        height: '85px',
                        objectFit: 'cover',
                        borderRadius: '8px',
                  ...
                  </div>) : col.logo ? (<img src={col.logo} alt={col.name} style={{
                    width: '60px',
                    height: '85px',
                    objectFit: 'cover',
                    borderRadius: '8px',
              ...
              {col.description && (<p style={{ margin: 0, fontSize: '0.85rem', color: '#a1a1aa', lineHeight: 1.5 }}>
                  {col.description}
                </p>)}
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={() => handleEditOpen(col)} style={{
                ...
                cursor: 'pointer'
                  ...
                  <button onClick={() => setDeletingCol(col)} style={{
      ...
      {/* CREATE MODAL */}
      {showCreateModal && (<ModalWrapper title="📦 Registrar Nueva Colección TCG" onClose={() => setShowCreateModal(false)}>
          <form onSubmit={handleCreateSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <ImageUploaderInput images={previewImages} onImageChange={handleImagesFileChange} onRemoveImage={handleRemoveImage}/>

            <div>
              <label style={labelStyle}>Nombre de la Colección</label>
              <input type="text" required placeholder="ej. Scarlet & Violet Base" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} style={inputStyle}/>
            </div>

            <div style={{ display: 'flex', gap: '1rem' }}>
              <div style={{ flex: 1 }}>
                <label style={labelStyle}>Código Único (Acrónimo)</label>
                <input type="text" required placeholder="ej. SV01" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} style={inputStyle}/>
              </div>
            ...
            <div>
              ...
              <input type="text" placeholder="ej. scarlet-violet-base" value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} style={inputStyle}/>
            </div>

            <div>
              <label style={labelStyle}>Descripción</label>
              <textarea rows={3} placeholder="Descripción detallada de la colección..." value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} style={{ ...inputStyle, resize: 'vertical' }}/>
            </div>
      ...
      {/* EDIT MODAL */}
      {editingCol && (<ModalWrapper title={`Editar Colección: ${editingCol.name}`} onClose={() => setEditingCol(null)}>
          <form onSubmit={handleEditSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="text" required value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} style={inputStyle}/>
              </div>
      ...
      {deletingCol && (<ModalWrapper title="Confirmar Eliminación" onClose={() => setDeletingCol(null)}>
...
function ImageUploaderInput({ images, onImageChange, onRemoveImage, }) {
    return (<div>
      ...
      <div style={{
        ...
        {images.length > 0 ? (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(90px, 1fr))', gap: '0.6rem' }}>
            {images.map((image, index) => (<div key={`${image}-${index}`} style={{ position: 'relative' }}>
                ...
                <button type="button" onClick={() => onRemoveImage(index)} style={{
                    position: 'absolute',
                    top: '0.3rem',
                    right: '0.3rem',
                    border: 'none',
        ...
        <input type="file" accept="image/*" multiple onChange={onImageChange} style={{
            position: 'absolute',
            top: 0,
            left: 0,
            width: '100%',
