...
export default function StudioCMSPage() {
  const [selectedShader, setSelectedShader] = useState('basic-foil');
  const [intensity, setIntensity] = useState(60);
  const [foilPattern, setFoilPattern] = useState('linear-rainbow');
  const [particleSpeed, setParticleSpeed] = useState(1.5);
  const [colorTint, setColorTint] = useState('#a855f7');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1.5rem' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <select
              value={foilPattern}
              onChange={(e) => setFoilPattern(e.target.value)}
              style={{
                width: '100%',
                padding: '0.65rem',
                borderRadius: '6px',
          ...
          <button
            onClick={() => alert('Preset guardado exitosamente en la base de datos')}
            style={{
              marginTop: '1rem',
              width: '100%',
              padding: '0.75rem',
