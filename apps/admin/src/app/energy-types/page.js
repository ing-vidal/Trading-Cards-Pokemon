...
exports.default = EnergyTypesAdminPage;
...
function IconPreview({ icon, color }) {
    if (!icon)
        return <span style={{ color: '#3f3f46', fontSize: '1.4rem' }}>—</span>;
    const isImage = icon.startsWith('data:image') || icon.startsWith('http');
    return isImage ? (<img src={icon} alt="icon" style={{ width: '32px', height: '32px', objectFit: 'contain', borderRadius: '4px' }}/>) : (<span style={{ fontSize: '1.5rem', lineHeight: 1 }}>{icon}</span>);
}
...
function EnergyTypesAdminPage() {
    const [energyTypes, setEnergyTypes] = (0, react_1.useState)([]);
    ...
    (0, react_1.useEffect)(() => { fetchEnergyTypes(); }, []);
    const openCreate = () => {
        setEditingId(null);
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', paddingBottom: '3rem' }}>
      ...
      {loading ? (<div style={{ textAlign: 'center', padding: '3rem', color: '#71717a' }}>Cargando tipos de energía...</div>) : energyTypes.length === 0 ? (<div style={{ textAlign: 'center', padding: '3rem', backgroundColor: '#18181b', borderRadius: '14px', border: '1px solid #27272a', color: '#71717a' }}>
          No hay tipos de energía. Agrega el primero con el botón de arriba.
        </div>) : (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem' }}>
          {energyTypes.map(et => (<div key={et.id} style={{
      ...
      {/* CREATE / EDIT MODAL */}
      {showModal && (<div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '16px', width: '100%', maxWidth: '520px', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              ...
              <div>
                ...
                <input type="text" required placeholder="ej. Agua" value={form.name} onChange={e => handleNameChange(e.target.value)} style={inputStyle}/>
              </div>

              {/* Slug (auto) */}
              <div>
                <label style={labelStyle}>Slug (identificador único)</label>
                <input type="text" placeholder="ej. agua" value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))} style={{ ...inputStyle, color: '#71717a' }}/>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.6rem' }}>
                  {['emoji', 'image'].map(mode => (<button key={mode} type="button" onClick={() => setForm(p => ({ ...p, iconMode: mode, icon: '' }))} style={{
                ...
                {form.iconMode === 'emoji' ? (<input type="text" placeholder="ej. 💧  🔥  ⚡  🌿" value={form.icon} onChange={e => setForm(p => ({ ...p, icon: e.target.value }))} style={{ ...inputStyle, fontSize: '1.2rem' }}/>) : (<div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    ...
                    {form.icon ? (<img src={form.icon} alt="preview" style={{ width: '48px', height: '48px', objectFit: 'contain', borderRadius: '8px', border: '1px solid #3f3f46' }}/>) : (<span style={{ color: '#71717a', fontSize: '0.8rem' }}>Sin imagen seleccionada</span>)}
                  </div>)}
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <input type="color" value={form.color} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} style={{ width: '48px', height: '40px', padding: '0', border: '1px solid #3f3f46', borderRadius: '8px', cursor: 'pointer', backgroundColor: 'transparent' }}/>
                  <input type="text" value={form.color} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} placeholder="#3b82f6" style={{ ...inputStyle, flex: 1 }}/>
                  {/* Live card preview */}
                  <div style={{
              ...
              {error && (<div style={{ backgroundColor: '#7f1d1d20', border: '1px solid #7f1d1d', borderRadius: '8px', padding: '0.75rem 1rem', color: '#fca5a5', fontSize: '0.85rem' }}>
                  ⚠️ {error}
                </div>)}
      ...
      {/* DELETE CONFIRM */}
      {deletingId && (<div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
