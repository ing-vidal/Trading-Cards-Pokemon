...
exports.default = RaritiesAdminPage;
...
function serializeIcons(icons) {
    const filtered = icons.filter(Boolean);
    if (filtered.length === 0)
        return null;
    if (filtered.length === 1)
        return filtered[0]; // backward compat: single icon stored as plain string
    return JSON.stringify(filtered);
}
/** Renders a single icon (image or emoji) */
function SingleIcon({ src, size = 28 }) {
...
function RarityIconRow({ icon, size = 28 }) {
    const icons = parseIcons(icon);
    if (icons.length === 0)
        return <span style={{ color: '#3f3f46', fontSize: '1.2rem' }}>—</span>;
...
function ImageSlot({ index, value, onChange, }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem' }}>
      ...
      <div onClick={() => ref.current?.click()} style={{
        ...
        {value ? (<img src={value} alt={`icon ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'contain' }}/>) : (<span style={{ fontSize: '1.5rem', color: '#3f3f46' }}>＋</span>)}
        <input ref={ref} type="file" accept="image/*" onChange={handleFile} style={{ display: 'none' }}/>
      </div>
      {value && (<button type="button" onClick={() => onChange(null)} style={{
...
// ─── Main page ────────────────────────────────────────────────────────────────
function RaritiesAdminPage() {
    const [rarities, setRarities] = (0, react_1.useState)([]);
    ...
    (0, react_1.useEffect)(() => { fetchRarities(); }, []);
    const openCreate = () => {
        setEditingId(null);
    ...
    const openEdit = (r) => {
        ...
        if (hasImages) {
            icons.forEach((img, idx) => { if (idx < 4)
                slots[idx] = img; });
        }
        setForm({
            name: r.name,
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', paddingBottom: '3rem' }}>
      ...
      <div style={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '14px', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
          ...
          <tbody>
            {loading ? (<tr><td colSpan={6} style={{ padding: '3rem', textAlign: 'center', color: '#71717a' }}>Cargando rarezas...</td></tr>) : rarities.length === 0 ? (<tr><td colSpan={6} style={{ padding: '3rem', textAlign: 'center', color: '#71717a' }}>No hay rarezas. Agrega la primera.</td></tr>) : rarities.map(r => (<tr key={r.id} style={{ borderBottom: '1px solid #27272a' }}>
          ...
          <div style={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '16px', width: '100%', maxWidth: '560px', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              ...
              <div>
                ...
                <input type="text" required placeholder="ej. Double Rare" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} style={inputStyle}/>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.85rem' }}>
                  {['emoji', 'images'].map(mode => (<button key={mode} type="button" onClick={() => setForm(p => ({ ...p, iconMode: mode }))} style={{
                ...
                {form.iconMode === 'emoji' ? (<input type="text" placeholder="ej. ◆◆◆  ⭐⭐  👑  ✦✦" value={form.emojiIcon} onChange={e => setForm(p => ({ ...p, emojiIcon: e.target.value }))} style={{ ...inputStyle, fontSize: '1.3rem' }}/>) : (<div>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                      {form.imageSlots.map((slot, idx) => (<ImageSlot key={idx} index={idx} value={slot} onChange={val => updateSlot(idx, val)}/>))}
                    </div>
                  </div>)}
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  ...
                  <input type="text" value={form.color} placeholder="#a855f7" onChange={e => setForm(p => ({ ...p, color: e.target.value }))} style={{ ...inputStyle, flex: 1 }}/>
