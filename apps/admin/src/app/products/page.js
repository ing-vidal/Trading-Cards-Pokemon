...
function AdminProductsPage() {
    ...
    const [cardsList, setCardsList] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [searchTerm, setSearchTerm] = (0, react_1.useState)('');
    const [conditionFilter, setConditionFilter] = (0, react_1.useState)('ALL');
    ...
    const [deletingProduct, setDeletingProduct] = (0, react_1.useState)(null);
    // Form State
        ...
        try {
            ...
            if (resProd.ok) {
                ...
                if (Array.isArray(prodJson)) {
                    const apiProducts = prodJson.map((p) => ({
                        id: p.id,
                        cardId: p.cardId,
            ...
            if (resCards.ok) {
                ...
                if (cardsJson.data && Array.isArray(cardsJson.data)) {
                    setCardsList(cardsJson.data.map((c) => ({ id: c.id, name: c.name, number: c.number })));
                }
            }
        }
        catch (e) {
    ...
    const filteredProducts = products.filter((p) => {
        const matchesSearch = p.cardName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', paddingBottom: '3rem' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 2, minWidth: '240px' }}>
          <input type="text" placeholder="🔍 Buscar por carta, SKU o condición..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{
            width: '100%',
        ...
        <div style={{ flex: 1, minWidth: '160px' }}>
          <select value={conditionFilter} onChange={(e) => setConditionFilter(e.target.value)} style={{
            width: '100%',
            backgroundColor: '#09090b',
            border: '1px solid #3f3f46',
            borderRadius: '8px',
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
          ...
          <tbody>
            {loading ? (<tr>
              ...
              </tr>) : filteredProducts.length === 0 ? (<tr>
                <td colSpan={7} style={{ padding: '3rem', textAlign: 'center', color: '#71717a' }}>
                  No hay productos registrados en el sistema.
                </td>
              </tr>) : (filteredProducts.map((prod) => (<tr key={prod.id} style={{ borderBottom: '1px solid #27272a' }}>
                  <td style={{ padding: '1rem 1.25rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      {prod.imageUrl ? (<img src={prod.imageUrl} alt={prod.cardName} style={{ width: '38px', height: '52px', borderRadius: '4px', objectFit: 'cover' }}/>) : (<div style={{ width: '38px', height: '52px', borderRadius: '4px', backgroundColor: '#27272a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem' }}>🎴</div>)}
                ...
                color: prod.status === 'AVAILABLE' && prod.stock > 0 ? '#34d399' : '#fca5a5'
                  ...
                  <td style={{ padding: '1rem 1.25rem', textAlign: 'right' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      <button onClick={() => handleEditOpen(prod)} style={{
                ...
                cursor: 'pointer'
                      ...
                      <button onClick={() => setDeletingProduct(prod)} style={{
      ...
      {/* CREATE MODAL */}
      {showCreateModal && (<ModalWrapper title="🏷️ Agregar Nuevo Listing de Producto" onClose={() => setShowCreateModal(false)}>
          <form onSubmit={handleCreateSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div>
              ...
              <select value={formData.cardId} onChange={(e) => setFormData({ ...formData, cardId: e.target.value })} style={inputStyle} required>
                <option value="">-- Selecciona una carta --</option>
                {cardsList.map((c) => (<option key={c.id} value={c.id}>
                    {c.name} (#{c.number})
                  </option>))}
              </select>
            </div>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <input type="number" step="0.01" value={formData.price} onChange={(e) => setFormData({ ...formData, price: Number(e.target.value) })} style={inputStyle} required/>
              </div>
            </div>
      ...
      {/* EDIT MODAL */}
      {editingProduct && (<ModalWrapper title={`Editar Product Listing: ${editingProduct.cardName}`} onClose={() => setEditingProduct(null)}>
      ...
      {deletingProduct && (<ModalWrapper title="Confirmar Eliminación" onClose={() => setDeletingProduct(null)}>
