...
exports.default = CardDetailPage;
...
const GoldCard3DCanvas = (0, dynamic_1.default)(() => import('@tcg/ui').then((mod) => mod.GoldCard3DCanvas), { ssr: false });
function mapRarityToPreset(rarityName) {
    if (!rarityName)
        return 'basic-foil';
    const r = rarityName.toLowerCase();
    if (r.includes('2-star') || r.includes('2 star'))
        return 'two-star-foil';
    if (r.includes('1-star') || r.includes('1 star'))
        return 'star-foil';
    if (r.includes('double rare'))
        return 'double-rare-foil';
    if (r === 'rare')
        return 'rare-foil';
    if (r.includes('rainbow') || r.includes('hyper'))
        return 'rainbow-hyper';
    if (r.includes('gold'))
        return 'gold-relic';
    if (r.includes('crown'))
        return 'crown-rare';
    if (r.includes('double immersive'))
        return 'double-immersive-rare';
    if (r.includes('immersive'))
        return 'immersive-rare';
    if (r.includes('secret') || r.includes('diamond'))
        return 'glass-shatter';
    if (r.includes('promo'))
        return 'promo-glow';
    if (r.includes('special') || r.includes('illustration'))
        return 'special-art';
    if (r.includes('trainer') || r.includes('gallery'))
        return 'trainer-gallery';
    if (r.includes('2-star') || r.includes('2 star'))
        return 'rainbow-hyper';
    return 'basic-foil';
}
function normalizeCollectionImages(value) {
...
function CardDetailPage({ params }) {
    const resolvedParams = react_1.default.use(params);
        ...
        async function loadCardDetails() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data && data.id) {
                        ...
                        function mapLevelToPreset(level) {
                            if (level === 'COMMON' || level === 'UNCOMMON')
                                return 'basic-foil';
                            if (level === 'RARE')
                                return 'rare-foil';
                            if (level === 'DOUBLE_RARE')
                                return 'double-rare-foil';
                            if (level === 'STAR_1')
                                return 'star-foil';
                            if (level === 'STAR_2')
                        ...
                        const collectionImages = normalizeCollectionImages(data.collection?.images
            ...
            try {
                ...
                if (saved) {
                    ...
                    const found = parsed.find((c) => c.id === cardId || c.name.toLowerCase() === cardId.toLowerCase());
                    if (found) {
                        const rarityName = found.rarity || '1-Star Rare';
                        const collectionImages = normalizeCollectionImages(found.images
    ...
    if (loading) {
        return (<div style={{ minHeight: '100vh', backgroundColor: '#09090b', color: '#f4f4f5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    if (!card) {
        return (<div style={{ minHeight: '100vh', backgroundColor: '#09090b', color: '#f4f4f5', padding: '2rem' }}>
        <div style={{ maxWidth: '600px', margin: '4rem auto', textAlign: 'center' }}>
          ...
          <link_1.default href="/catalog" style={{ backgroundColor: '#38bdf8', color: '#09090b', padding: '0.65rem 1.25rem', borderRadius: '8px', fontWeight: 700, textDecoration: 'none' }}>
            ← Volver al Catálogo
          </link_1.default>
        </div>
      </div>);
    ...
    return (<div className="detail-page" style={{ minHeight: '100vh', backgroundColor: '#09090b', color: '#f4f4f5', padding: 'clamp(1rem, 4vw, 2rem)' }}>
      <div className="detail-shell" style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 'clamp(1.25rem, 3vw, 2rem)' }}>
        ...
        <div className="detail-breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', color: '#a1a1aa' }}>
          <link_1.default href="/catalog" style={{ color: '#38bdf8', textDecoration: 'none' }}>Catálogo</link_1.default>
          <span>/</span>
        ...
        <GoldCard3DCanvas key="gold-premium" imageUrl={card.imageUrl} intensity={0.9} width="100%" height="100%" showEditor={false}/>) : (
        /* Standard shader renderer for all other rarities */
        <Card3DCanvas key={shaderPresetId} imageUrl={card.imageUrl} presetId={shaderPresetId} intensity={0.85} height="100%"/>)}
            </div>
          ...
          <div className="tech-column" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', minWidth: 0 }}>
            ...
            gap: '1.25rem',
              ...
              {card.collectionImages && card.collectionImages.length > 0 ? (<div className="expansion-gallery" style={{ display: 'flex', gap: '1rem', flexWrap: 'nowrap', alignItems: 'flex-start', minWidth: 0 }}>
                  {card.collectionImages.slice(0, 3).map((image, index) => (<img key={`${image}-${index}`} src={image} alt={`Expansión ${index + 1} de ${card.collection}`} className="expansion-image" style={{
                    width: 'min(28%, 180px)',
                    height: 'auto',
                    maxHeight: '240px',
                    objectFit: 'contain',
            ...
            <div style={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '12px', padding: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {card.prices.map((p, idx) => (<div className="price-row" key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 0.8rem', backgroundColor: '#09090b', borderRadius: '6px' }}>
                    <div>
