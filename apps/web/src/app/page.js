...
function IconDisplay({ icon, size = 18 }) {
    const icons = parseIcons(icon);
    if (icons.length === 0)
        return null;
    return (<span style={{ display: 'inline-flex', alignItems: 'center', gap: '1px' }}>
      {icons.map((src, i) => {
...
function IconSelect({ label, placeholder, value, options, onChange, }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
      ...
      <div ref={ref} style={{ position: 'relative' }}>
        ...
        {open && (<div style={{
            ...
            {/* "All" option */}
            <button type="button" onClick={() => { onChange('ALL'); setOpen(false); }} style={{
                width: '100%',
                padding: '0.6rem 1rem',
                border: 'none',
            ...
            {options.map(opt => (<button key={opt.id} type="button" onClick={() => { onChange(opt.id); setOpen(false); }} style={{
                ...
                <span>{opt.name}</span>
                {opt.color && (<span style={{ marginLeft: 'auto', width: 10, height: 10, borderRadius: '50%', backgroundColor: opt.color, flexShrink: 0 }}/>)}
              </button>))}
          </div>)}
      </div>
    </div>);
...
function PublicHomePage() {
    ...
    const [collectionsList, setCollectionsList] = (0, react_1.useState)([]);
    const [raritiesList, setRaritiesList] = (0, react_1.useState)([]);
    const [energyTypesList, setEnergyTypesList] = (0, react_1.useState)([]);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [search, setSearch] = (0, react_1.useState)('');
    const [selectedCollection, setSelectedCollection] = (0, react_1.useState)('Genes Formidables');
    const [selectedRarityId, setSelectedRarityId] = (0, react_1.useState)('ALL');
    const [selectedEnergyTypeId, setSelectedEnergyTypeId] = (0, react_1.useState)('ALL');
    const [selectedCardType, setSelectedCardType] = (0, react_1.useState)('ALL');
    (0, react_1.useEffect)(() => {
            ...
            try {
                if (resCol.status === 'fulfilled' && resCol.value.ok) {
                    ...
                    if (Array.isArray(json) && json.length > 0)
                        setCollectionsList(json.map((c) => ({ id: c.id, name: c.name, code: c.code })));
                }
            }
            catch (e) {
                console.warn('Collections fetch error', e);
            }
            try {
                if (resRar.status === 'fulfilled' && resRar.value.ok) {
                    const json = await resRar.value.json();
                    if (Array.isArray(json)) {
                        const mapped = json.map((r) => ({ id: r.id, name: r.name, color: r.color, icon: r.icon, level: r.level }));
                ...
                console.warn('Rarities fetch error', e);
            }
            try {
                if (resEnergy.status === 'fulfilled' && resEnergy.value.ok) {
                    const json = await resEnergy.value.json();
                    if (Array.isArray(json))
                        setEnergyTypesList(json.map((e) => ({ id: e.id, name: e.name, icon: e.icon, color: e.color })));
                }
            }
            catch (e) {
                console.warn('Energy types fetch error', e);
            }
            let apiCards = [];
            try {
    ...
    return (<div style={{ minHeight: '100vh', backgroundColor: '#09090b', color: '#f4f4f5', padding: 'clamp(1rem, 4vw, 2rem)' }}>
      ...
      <header style={{
        ...
        <link_1.default href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{
        ...
        <nav style={{ display: 'flex', gap: '1.5rem', fontSize: '0.9rem' }}>
          <link_1.default href="/" style={{ color: '#38bdf8', fontWeight: 600, textDecoration: 'none' }}>Catálogo</link_1.default>
          <link_1.default href="/checkout" style={{ color: '#a1a1aa', textDecoration: 'none' }}>Marketplace</link_1.default>
          <link_1.default href="/orders" style={{ color: '#a1a1aa', textDecoration: 'none' }}>Mis Pedidos</link_1.default>
        </nav>
      </header>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input type="text" placeholder="Buscar por nombre o número de carta..." value={search} onChange={e => setSearch(e.target.value)} style={{
            width: '100%',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
              ...
              <select value={selectedCollection} onChange={e => setSelectedCollection(e.target.value)} style={{
            padding: '0.65rem 2.5rem 0.65rem 1rem',
            borderRadius: '8px',
            border: '1px solid #3f3f46',
            backgroundColor: '#09090b',
            ...
            backgroundPosition: 'right 12px center',
                ...
                {collectionsList.map(col => (<option key={col.id} value={col.name}>{col.name} ({col.code})</option>))}
              </select>
            </div>

            {/* Rarity — custom dropdown with image icon support */}
            <IconSelect label="⭐ Rareza" placeholder="Todas las Rarezas" value={selectedRarityId} options={raritiesList} onChange={setSelectedRarityId}/>

            {/* Energy Type — custom dropdown with image icon support */}
            <IconSelect label="⚡ Tipo de Energía" placeholder="Todos los Tipos" value={selectedEnergyTypeId} options={energyTypesList} onChange={setSelectedEnergyTypeId}/>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
              <label style={{ fontSize: '0.75rem', color: '#71717a', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                🃏 Tipo de Carta
              </label>
              <select value={selectedCardType} onChange={e => setSelectedCardType(e.target.value)} style={{
            padding: '0.65rem 2.5rem 0.65rem 1rem',
            borderRadius: '8px',
            border: '1px solid #3f3f46',
            backgroundColor: '#09090b',
          ...
          {/* Active filter pills */}
          {hasActiveFilters && (<div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '0.8rem', color: '#71717a' }}>Filtros activos:</span>
              {search && (<span style={{ fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '99px', backgroundColor: '#38bdf820', color: '#38bdf8', border: '1px solid #38bdf840' }}>
                  🔍 "{search}"
                </span>)}
              {selectedCollection !== 'ALL' && (<span style={{ fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '99px', backgroundColor: '#a855f720', color: '#c084fc', border: '1px solid #a855f740' }}>
                  📦 {selectedCollection}
                </span>)}
              {selectedRarityId !== 'ALL' && (<span style={{ fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '99px', backgroundColor: '#f59e0b20', color: '#fbbf24', border: '1px solid #f59e0b40', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  <IconDisplay icon={raritiesList.find(r => r.id === selectedRarityId)?.icon} size={14}/>
                  {raritiesList.find(r => r.id === selectedRarityId)?.name}
                </span>)}
              {selectedEnergyTypeId !== 'ALL' && (<span style={{ fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '99px', backgroundColor: '#10b98120', color: '#34d399', border: '1px solid #10b98140', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
              ...
              {selectedCardType !== 'ALL' && (<span style={{ fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '99px', backgroundColor: '#f43f5e20', color: '#fb7185', border: '1px solid #f43f5e40' }}>
        ...
        {/* Result count */}
        {!loading && (<div style={{ fontSize: '0.875rem', color: '#71717a' }}>
        ...
        {/* Cards grid */}
        {loading ? (<div style={{ textAlign: 'center', padding: '4rem', color: '#a1a1aa' }}>Cargando cartas del catálogo...</div>) : filteredCards.length === 0 ? (<div style={{ textAlign: 'center', padding: '4rem', backgroundColor: '#18181b', borderRadius: '14px', border: '1px solid #27272a' }}>
          ...
          </div>) : (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1.5rem' }}>
            {filteredCards.map(card => (<link_1.default key={card.id} href={`/cards/${card.id}`} style={{ textDecoration: 'none' }}>
                <div style={{
                    backgroundColor: '#18181b',
                    border: '1px solid #27272a',
                    borderRadius: '14px',
                  ...
                  <div>
                    ...
                    {card.energyType && (<div style={{ fontSize: '0.75rem', color: card.energyTypeColor || '#a1a1aa', marginTop: '0.15rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
